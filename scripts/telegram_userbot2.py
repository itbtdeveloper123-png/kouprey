...
async def main():
    ...
    # 1. If api_id or api_hash is missing, attempt to fetch from Admin Panel API automatically
    admin_api_url = config.get("admin_api_url", "")
    if (not api_id or not api_hash) and admin_api_url:
        logger.info("🔍 កំពុងស្វែងរក api_id និង api_hash ពី Admin Panel API...")
        admin_cfg = fetch_admin_settings(admin_api_url)
        if admin_cfg:
            if admin_cfg.get("telegram_api_id"):
                api_id = admin_cfg.get("telegram_api_id")
                config["api_id"] = api_id
            if admin_cfg.get("telegram_api_hash"):
                api_hash = admin_cfg.get("telegram_api_hash")
                config["api_hash"] = api_hash
            if admin_cfg.get("telegram_phone_number"):
                phone = admin_cfg.get("telegram_phone_number")
                config["phone_number"] = phone
            try:
                with open(CONFIG_FILE, "w", encoding="utf-8") as f:
                    json.dump(config, f, indent=4, ensure_ascii=False)
                logger.info("✓ បានទាញយក api_id និង api_hash ពី Admin Panel ដោយជោគជ័យ!")
            except Exception:
                pass
